...
「起動すると即録音、ボタン一つで送信」を実現する、常駐しない Windows 用の小型音声メモツール。
録音した音声は任意の Webhook へ HTTP POST する。特定のサービス（n8n 等）に依存せず、URL を差し替えるだけで送信先を変えられる。

HTTP リクエストは `reqwest` でシェルを介さず直接組み立てる（外部 curl 等に依存しない）。

## 2. プロセスモデル

**都度起動型**。常駐・トレイ・トグル状態機械は持たない。

1. exe を起動する（ショートカットやタスクバーからの起動でもよい）。
2. 起動と同時に即録音開始し、小さなウィンドウを最前面に表示。
3. ウィンドウの「送信」ボタンで Webhook へ生バイナリ POST。
...
2. デバイス存在チェック（`TARGET_DEVICE_NAME` が指定されている場合のみ）: `cpal::default_host().input_devices()` を列挙し、名前に部分一致するデバイスを探す。
   ...
   - **未指定**: チェックを行わず、OS の既定の入力デバイス（`default_input_device()`）で録音する。既定デバイスが 1 つも無ければ `Fatal`。
...
送信先は特定サービスに依存しない汎用の Webhook とする。

- メソッド: `POST`
- URL: `WEBHOOK_URL` をそのまま使う（アプリ側で加工しない）。クエリパラメータ（`?source=pc` 等）が必要なら URL 自体に含める。
- 認証: `BASIC_AUTH_USER` / `BASIC_AUTH_PASS` が設定されているときのみ `Authorization: Basic <base64>`（`reqwest` の `.basic_auth(user, Some(pass))` に委譲）。未設定なら認証ヘッダを付けない。
...
| キー | 必須 | 内容 |
| --- | --- | --- |
| `WEBHOOK_URL` | 必須 | 送信先 Webhook の URL。 |
| `BASIC_AUTH_USER` / `BASIC_AUTH_PASS` | 任意 | Basic 認証。両方揃っていれば付与、両方空なら認証なし。片方だけならエラー（401 の原因が分かりにくいため）。 |
| `TARGET_DEVICE_NAME` | 任意 | 録音に使う入力デバイス名（部分一致）。未指定なら既定デバイス。 |
| `AUDIO_BITRATE_KBPS` | 任意 | Opus の目標ビットレート。未設定なら 64、範囲 6〜510 にクランプ。 |
...
2. 「送信」→「送信中…」→ Webhook が 2xx を返すとウィンドウが自動で閉じる。
3. `TARGET_DEVICE_NAME` 指定デバイスが未接続の状態で起動 → 録音されず「デバイスが見つかりません」表示、「閉じる」で終了。`TARGET_DEVICE_NAME` 未指定なら既定デバイスで録音が始まる。
...
補助検証: 送信部は Basic 認証 / 2xx 制御のため、実エンドポイントか、URL・ヘッダ・ボディをログ出力する簡易 HTTP スタブで確認する。
