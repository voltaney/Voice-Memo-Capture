...
- ドキュメント・コード・テスト・コミットメッセージに**実在の値を書かない**。Webhook URL・ホスト名・UUID・ユーザー名・パスワードは `example.com` / `XXXXXXXX` のようなプレースホルダにする（リクエスト例などで実物をそのまま載せない）。
- 画像を追加するときは EXIF / XMP（作成者名・撮影日時など）を除去してからコミットする。
- `.env` はコミットしない。設定例が必要なら `.env.example` に書く。
